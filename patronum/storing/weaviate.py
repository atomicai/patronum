...
from patronum.etc import Document, FilterType, Label, LogicalFilterClause, get_batches_from_generator
from patronum.storing import IDocStore
...
class KeywordDocumentStore(IDocStore):
    """
...
class IWeaviateDocStore(KeywordDocumentStore):
    ...
    def __init__(
        ...
        try:
            ...
            if not status:
                raise ConnectionError(
                    "Initial connection to Weaviate failed. Make sure you run Weaviate instance "
        ...
        if similarity == "cosine":
            ...
            raise Exception(
                f"It looks like you provided value '{similarity}' for similarity in the WeaviateDocumentStore"
                " constructor. Choose one of these values: 'cosine', 'l2', and 'dot_product'"
            )
                ...
                "classes": [
                    {
                        ...
                        "invertedIndexConfig": {"cleanupIntervalSeconds": 60, "bm25": {"b": 0.75, "k1": 1.2}},
                        "vectorizer": "none",
                        "properties": [
        ...
        if not self.weaviate_client.schema.contains(schema):
            ...
            if _db_similarity != self.similarity:
                raise ValueError(
                    ...
                    "If there is a Weaviate index created with a certain similarity, you can't "
                    "query with a different similarity. If you need a different similarity, "
                    "recreate the index. To do this, set the `recreate_index=True` argument."
                )
    ...
    def _convert_weaviate_result_to_document(
        ...
        if "_additional" in props:
            if "score" in props["_additional"]:
                _score = props["_additional"]["score"]
    ...
    def query(
        ...
        # TODO: Integrate search by embedding here as well
        results = []
    ...
    def update_embeddings(
        self,
        retriever: Optional[nn.Module] = None,
        index: Optional[str] = None,
        filters: Optional[FilterType] = None,
