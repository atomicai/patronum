...
class ICLSHead(IFlow):
    def __init__(
        ...
        **kwargs,
        ...
        if type(class_weights) is np.ndarray and class_weights.ndim != 1:
            raise ValueError(
                "When you pass `class_weights` as `np.ndarray` it must have 1 dimension! You provided {} dimensions."
                .format(class_weights.ndim)
            )
    ...
    def formatted_preds(self, logits=None, preds=None, samples=None, return_class_probs=False, **kwargs):
        ...
        # This case covers Natural Questions where the sample is in a QA style
            ...
            contexts = [
                sample.clear_text["question_text"] + " | " + sample.clear_text["passage_text"] for sample in samples
            ]
        ...
        if (
            ...
            if n_added_tokens == 0:
                ...
                bias_params = torch.nn.Parameter(
                    torch.cat([bert_with_lm.cls.predictions.bias, torch.zeros(n_added_tokens)])
                )
