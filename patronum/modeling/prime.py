...
class IDIBERT(ILanguageModel):
    ...
    def __init__(self):
        self.name = "IDIBERT"
        super(IDIBERT, self).__init__(model_type=self.name)
        self.model = None
        # self.name = "IDIBERT"
        self.pooler = None
...
class IE5Model(ILanguageModel):
    def __init__(self):
        self.name = "e5"
        super(IE5Model, self).__init__(self.name)
        self.model = None
    ...
    def forward(
        ...
        """
        output_tuple = self.model(input_ids=input_ids, attention_mask=attention_mask, segment_ids=segment_ids)
        # We need to manually aggregate that to get a pooled output (one vec per seq)
        pooled_output = self.pooler(output_tuple[0])
        if self.model.config.output_hidden_states is True:
            sequence_output, _ = output_tuple[0], output_tuple[1]
