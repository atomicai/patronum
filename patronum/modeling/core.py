...
from patronum.modeling.flow import IFlow
from patronum.modeling.prime import ILanguageModel
...
class ICoreModel:
    ...
    def load(cls, **kwargs):
        """
        Load corresponding M1 or ONNX (not yet) Class(AdaptiveModel/ONNXAdaptiveModel) based on the
        ...
        if (Path(kwargs["load_dir"]) / "model.onnx").is_file():
            ...
            model = cls.subclasses["M1Model"].load(**kwargs)
        return model
